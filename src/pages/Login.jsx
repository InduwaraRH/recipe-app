    ...
    <div className="flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-md p-8">
        ...
        <div className="mt-6 flex gap-6 justify-center text-sm">
          <span className="pb-2 border-b-2 border-indigo-600">Login</span>
          <Link className="pb-2 border-b-2 border-transparent hover:border-indigo-600" to="/register">Register</Link>
        </div>
        <motion.form onSubmit={submit} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="mt-6 space-y-4">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2 rounded-xl bg-indigo-600 text-white disabled:opacity-50">{loading?'Please wait…':'Log in'}</button>
        </motion.form>
      </div>
    </div>
  )
